...
> Thank you for your interest in Chrome Resizer Extension!
> Contributions of all kinds are welcome — from fixing typos to proposing entirely new features.
...
git clone https://github.com/<your-username>/chrome-resizer-extension.git
cd chrome-resizer-extension
...
git commit -m "feat(popup): add custom preset support"
...
This repository follows **Conventional Commits** with the `feat` or `docs` type as default.

```
docs(<scope>): <subject>
```

**Scopes**: `popup`, `background`, `css`, `docs`, `config`

```bash
# Examples
git commit -m "feat(popup): add custom resolution preset support"
git commit -m "fix(background): correct hotkey behavior"
...
By contributing, you agree that your contributions will be licensed under the [MIT License](LICENSE) license.
